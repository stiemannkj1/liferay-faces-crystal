...
public class SelectRatingRenderer extends SelectRatingRendererBase {
	...
	public void encodeJavaScriptCustom(FacesContext facesContext, UIComponent uiComponent) throws IOException {
		...
		// no developer attribute is allowed for this component, even though crystalui allows for an attribute called defaultSelected
		...
		// Initialize the hidden input generated by CrystalUI to the hiddenInputValue, instead of its default of -1.
		...
		if (selectedIndex.intValue() == SelectRatingResponseWriter.NO_SELECTION_INDEX) {
			...
			// anything, or select anything, since the Crystal component will simply be able to render itself correctly.
			else {
				// no operation needed.
			}
		}
		...
		//
		// This is the case where an crystalSelectedIndex has been established (or Crystal renders stars already
		// filled to the defaultSelected input), but the user has selected a different number of stars than the
		// crystalSelectedIndex, so we need to "select" the correct number of stars to be filled. If the crystalSelectedIndex is
		// the same as the indexSelectedByJSF, we do not want to select it again, since crystal has already filled the
		...
		//J+
			...
			responseWriter.write("var crystalSelectedIndex=");
			responseWriter.write(clientVarName);
			responseWriter.write(".get('selectedIndex');if(");

			responseWriter.write(indexSelectedByJSF);
			responseWriter.write("!=crystalSelectedIndex){");
			responseWriter.write(clientVarName);
		...
		//J-
			...
			// Contributor			   What value do they want?				What will we do with this value?
			// ----------------------  -----------------------------------	---------------------------------------------
			// 1. CrystalUI			   event.target.get('value')			Use this as the value unless it is -1 which is
			//																the cancel value.
			//
